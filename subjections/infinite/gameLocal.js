...
function makeBoard() {
  ...
  function canPlaceCombo(comboObj, targetCid) {
    ...
    for (let c = 0; c < nClasses; c++) {
      const inc = counts[c]; if (!inc) continue;
      const after = pickedClassCounts[c] + inc;
      if (c === targetCid) { if (after > 4) return false; } else { if (after > 3) return false; }
    }
    for (const s of comboObj.students) {
  ...
  function applyCombo(comboObj, targetCid) {
    for (const s of comboObj.students) {
      usedStudentFlag[s] = 1;
      const b = studentBase[s]; if (b) usedBaseCount[b] = (usedBaseCount[b] || 0) + 1;
  ...
  function revertCombo(comboObj, targetCid) {
    for (const s of comboObj.students) {
      usedStudentFlag[s] = 0;
      const b = studentBase[s]; if (b) { const v = (usedBaseCount[b] || 1) - 1; if (v <= 0) delete usedBaseCount[b]; else usedBaseCount[b] = v; }
      for (const c of studentClasses[s]) pickedClassCounts[c] -= 1;
    }
    pickedGroups.pop();
  }
  function validatePartial() {
    ...
    for (let c = 0; c < nClasses; c++) {
      ...
      if (isTarget) { if (cnt > 4) return false; } else { if (cnt >= 4) return false; }
    }
    return true;
  }
  ...
  function backtrack(classOrder, currentCombos, pos) {
    ...
    for (let i = pos; i < classOrder.length; i++) {
      ...
      let conflict = false;
      for (const s of classStudents[cid]) { if (usedStudentFlag[s]) { conflict = true; break; } }
      ...
      for (let ci = 0; ci < limit; ci++) {
        ...
        if (!validatePartial()) { revertCombo(comboObj, cid); continue; }
  ...
  const viableClasses = viableClassesBase.slice();
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    nodes = 0;
    for (let s = 0; s < nStudents; s++) usedStudentFlag[s] = 0;
    for (let c = 0; c < nClasses; c++) pickedClassCounts[c] = 0;
    for (const k of Object.keys(usedBaseCount)) delete usedBaseCount[k];
    pickedGroups.length = 0;

    if (backtrack(viableClasses, currentCombos, 0)) { found = true; break; }
  }
  ...
  shuffle(window.selectedStudents);
  ...
  for (let i = 0; i < 4; i++) {
    ...
    for (let j = 0; j < 4; j++) {
      ...
      const span = document.createElement('span'); span.className = 'cell-text'; btn.appendChild(span);
      fadeChangeText(btn, name);
      btn.addEventListener('click', () => {
        if (btn.classList.contains('toggle')) { btn.classList.remove('toggle'); window.toggledCount--; }
        else if (window.toggledCount < 4) { btn.classList.add('toggle'); window.toggledCount++; }
  ...
  setCookie("selectedGroupsSubjections", JSON.stringify(pickedGroups.map(g => ({ class: classNames[g.cid], students: g.comboObj.students.map(i => studentNames[i]) }))), 1);
}

makeBoard();

function fadeChangeText(button, newText) {
  const span = button.querySelector('.cell-text');
